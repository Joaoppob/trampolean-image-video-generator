...
- A pasta `RAG/identidade-visual/` tem imagens? (afeta "posso gerar agora?")
- O Higgsfield está conectado? (afeta "por que não gera?")
- Existe `output/.pipeline-state.json`? (afeta "tenho um run em andamento?")
...
- **"Onde coloco minhas imagens?"** Em `RAG/identidade-visual/`. De 1 a 3 imagens do mesmo
  personagem ou produto. São o que mantém a cara igual entre as cenas.
...
- **"Posso usar para outra coisa que não jogo?"** Sim. Os moldes cobrem produto e lifestyle
  também (e-commerce, serviço). Troque a identidade na pasta `RAG/`.
