...
1. **Catálogo VIVO** (consulta o Higgsfield real + destrava a geração). Este passo é
   **obrigatório por trava**: o hook bloqueia `higgsfield generate create` enquanto o catálogo
   não tiver sido consultado nesta sessão. Rode:

   ```bash
   node scripts/refresh-catalog.cjs
   ```

   Ele puxa `higgsfield model list --image` e `--video` reais, atualiza o cache (sem hardcode) e
   carimba o token de consulta. (Rodar `higgsfield model list` direto também conta, mas o
   `refresh-catalog` é o canônico — alimenta o advisor.)
...
2. **Parecer + custo por cenário** (lê o catálogo vivo; real pro default, AC pros pagos):
   ...
   O advisor traz `catalogo_fonte: "vivo"` e marca cada opção com `disponivel_no_higgsfield`
   conforme a lista real. Opção que não está no catálogo vivo não deve ser oferecida.
